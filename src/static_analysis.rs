use crate::types::{AnalysisReport, CrateInfo, SectionInfo};
use anyhow::Result;
...
pub fn analyze(path: &Path, data: &[u8]) -> Result<AnalysisReport> {
    ...
    let mut signature_publisher = None;
    ...
    let (format, architecture) = match Object::parse(data)? {
        Object::PE(pe) => {
            ...
            if let Some(opt) = pe.header.optional_header {
                ...
                // Check Security Data Directory (IMAGE_DIRECTORY_ENTRY_SECURITY index 4)
                if let Some(Some((_, sec_dir))) = opt.data_directories.data_directories.get(4) {
                    if sec_dir.virtual_address > 0 && sec_dir.size > 0 {
                        is_signed = true;
                        indicators.push("Digitally Signed PE (Authenticode)".to_string());

                        // Inspect certificate data if inside buffer
                        let offset = sec_dir.virtual_address as usize;
                        let size = sec_dir.size as usize;
                        if offset + size <= data.len() {
                            let cert_slice = &data[offset..offset + size];
                            let cert_str = String::from_utf8_lossy(cert_slice);
                            if cert_str.contains("Microsoft Corporation") {
                                signature_publisher = Some("Microsoft Corporation".to_string());
                            } else if cert_str.contains("Google LLC") {
                                signature_publisher = Some("Google LLC".to_string());
                            } else if cert_str.contains("Apple Inc.") {
                                signature_publisher = Some("Apple Inc.".to_string());
                            } else {
                                signature_publisher = Some("Signed (Valid Cert Data)".to_string());
                            }
                        } else {
                            signature_publisher = Some("Signed PE Binary".to_string());
                        }
                    }
                }
            }
    ...
    if has_resources {
        score += 4;
        if let Some(sz) = resource_size {
            if sz > 100_000 {
                score += 6;
                indicators.push(format!("Large resources detected ({} bytes)", sz));
            } else {
...
fn detect_packer_and_compiler(
    ...
    if sections.iter().any(|s| s.entropy >= 7.2) && findings.is_empty() {
